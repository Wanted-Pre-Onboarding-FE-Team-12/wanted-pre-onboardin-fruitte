...
export const ProductSelectList = ({ optionSelect, setOptionSelect, setTotalPrice }) => {
    const handleClick = (event) => {
        ...
        setOptionSelect(()=>{return updatedList})
    }
    
    const updateTotalPrice = () => {
        let newTotalPrice = optionSelect.reduce(
            function (prev, current) { return prev + current.count*current.price; },
            0
        )
        setTotalPrice(()=>{return newTotalPrice})
    }
    ...
    const handleChange = (event) => {
        ...
        setOptionSelect(()=> {return newOptionSelect})
        updateTotalPrice()
    }
    return (
        <SelectList>
            {optionSelect && optionSelect.map((option)=>(
